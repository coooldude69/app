...
# one row, mobile-friendly: search + small filters
...
# make safe copies of filter options
districts = (["All"] +
...
# Apply search (case-insensitive) over key columns
if search_text:
    s = search_text.strip()
    mask = (
...
# compact display: show RO name + SAP + TA
ro_display = (
...
with k1:
    ...
    st.metric("MS Avg 25-26", format_number(ms_avg))

with k2:
    st.metric("HSD 25-26 (KL)", format_number(hsd_latest))
    st.metric("HSD Avg 25-26", format_number(hsd_avg))

with k3:
    st.metric("TY MS+HSD (KL)", format_number(ty))
    st.metric("Peak MS (KL)", format_number(ms_peak))

with k4:
    st.metric("Peak HSD (KL)", format_number(hsd_peak))
...
with t_col1:
    st.caption("Monthly MS / HSD")
    if ms_cols and h_cols:
        ms_series = ro_row[ms_cols]
        hsd_series = ro_row[h_cols]

        min_len = min(len(ms_series), len(hsd_series))
        ms_series = ms_series.iloc[:min_len]
        hsd_series = hsd_series.iloc[:min_len]
        months = ms_series.index.to_list()

        ts = pd.DataFrame({
            "Month": months,
            "MS": ms_series.values,
            "HSD": hsd_series.values
        })
        ts_long = ts.melt("Month", var_name="Product", value_name="Volume")
        ts_long = ts_long.replace({np.nan: 0})
        st.line_chart(ts_long, x="Month", y="Volume", color="Product")
    else:
        st.info("Monthly MS/HSD columns not found.")
